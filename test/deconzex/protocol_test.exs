defmodule ProtocolTest do
  ...
  describe "encode requests" do
    ...
    test "write_parameter_request" do
      ...
      expected =
        <<command::8, seq::8, 0x00, frame_len::16-little, payload_len::16-little, parameter_id::8,
          addr::64-little, 0xCE, 0xFB>>

      assert Protocol.write_parameter_request(seq, :mac_address, addr) == expected
    end
    ...
    test "enqueue_send_data_request" do
      ...
      assert <<0x12, 0x10, 0x00, 28, 0x00, 0x15, 0x00, 12, 0x00, 0x02, 0xEF, 0xCD, 16, 0x34, 0x12,
               0x78, 0x56, 0x02, 0x06, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x04, 0x00, 158,
               252>> =
               Protocol.enqueue_send_data_request(
                 16,
                 req_id,
                 Address.nwk(dest_address),
                 dest_endpoint,
                 profile_id,
                 cluster_id,
                 source_endpoint,
    ...
    test "query_send_data_request" do
      assert <<0x04, 0x02, 0x00, 0x07, 0x00, 0x00, 0x00, 243::8, 255::8>> =
               Protocol.query_send_data_request(2)
    end
  end
  ...
  describe "decode responses" do
    ...
    test "read parameter response" do
      frame =
        <<0x0A, 0x12, 0x00, 0x10, 0x00, 0x09, 0x00, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
          0x07, 0x08, 0xA6, 0xFF>>
    ...
    test "read received data response: address mode 01 (group)" do
      ...
      src_address = Address.nwk(0xF134)
      dest_address = Address.group(0xFFBB)
    ...
    test "read received data response: address mode 02 (nwk)" do
      ...
      dest_address = Address.nwk(0xFFBB)
      source_address = Address.nwk(0xF134)
    ...
    test "read received data response: address mode 03 (ieee)" do
      ...
      assert [
               %{
                 destination_address: {:ieee, 0xFFBB0000},
                 destination_endpoint: 1,
                 source_address: {:ieee, 0xF1340000},
                 source_endpoint: 2
               }
             ] = Protocol.decode(frame)
    end
    ...
    test "read received data response: address mode 04 (nwk and ieee)" do
      ...
      assert [
               %{
                 destination_address: {:ieee, 0xFFBB0000},
                 destination_endpoint: 1,
                 source_address: [{:nwk, 0xCDEF}, {:ieee, 0xF1340000}],
                 source_endpoint: 2
               }
             ] = Protocol.decode(frame)
    end
    ...
    test "mac poll indication with no life time" do
      frame =
        <<0x1C, 0x01, 0x00, 0x0C, 0x00, 0x04, 0x00, 0x02, 0xABCD::16-little, 0xE1, -30::8-signed,
          0x96, 0xFC>>
    ...
    test "mac poll indication with lifetime and ieee address" do
      ...
      assert [
               %{
                 command: :mac_poll_indication,
                 seq: 1,
                 source_address: {:ieee, 0x1234ABCD},
                 lqi: 225,
                 rssi: -30,
                 lifetime: 200,
                 timeout: 1000
    ...
    test "mac beacon indication" do
      frame =
        <<0x1F, 0x40, 0x00, 22, 0x00, 0x00, 0x00, 0xCDEF::16-little, 0x1234::16-little, 0x20,
          0x01, 0x10, 0x11111111::64-little, 0x14, 0xFD>>
    ...
    test "enque send data response" do
      frame = <<0x12, 0x20, 0x00, 0x09, 0x00, 0x00, 0x02, 0b00000110, 0x23, 0x9A, 0xFF>>
    ...
    test "query send data state response with address mode 1" do
      frame =
        <<0x04, 0x02, 0x00, 0x12, 0x00, 0x0B, 0x00, 0b00000110, 0x23, 0x01, 0xEF, 0xCD, 0x02,
          0x01, 0::32, 0xF4, 0xFD>>
      ...
      assert [
               %{
                 ...
                 configuration_changed: 0,
                 request_id: 35,
                 destination_address: {:group, 0xCDEF},
                 source_endpoint: 2,
                 confirm_status: 1
               }
             ] = Protocol.decode(frame)
    ...
    test "query send data state response with address mode 2" do
      frame =
        <<0x04, 0x02, 0x00, 0x13, 0x00, 0x0C, 0x00, 0b00000110, 0x23, 0x02, 0xEF, 0xCD, 0x02,
          0xF0, 0x01, 0::32, 0x01, 0xFD>>
      ...
      assert [
               %{
                 ...
                 configuration_changed: 0,
                 request_id: 35,
                 destination_address: {:nwk, 0xCDEF},
    ...
    test "query send data state response with address mode 3" do
      frame =
        <<0x04, 0x02, 0x00, 0x19, 0x00, 0xF2, 0x00, 0b00000110, 0x23, 0x03, 0x78, 0x56, 0x34,
          0x12, 0x78, 0x56, 0x34, 0x12, 0x02, 0xF0, 0x01, 0::32, 0xA8, 0xFB>>
      ...
      assert [
               %{
                 ...
                 configuration_changed: 0,
                 request_id: 35,
                 destination_address: {:ieee, 0x1234567812345678},
                 destination_endpoint: 2,
                 source_endpoint: 240,
                 confirm_status: 1
               }
